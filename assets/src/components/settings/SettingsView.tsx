      ...
      {activeTab === 'backup' && (
        <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Neon Serverless PostgreSQL - Netlify Partner */}
            <div className="p-5 bg-gradient-to-br from-emerald-950 via-forest-950 to-slate-900 border border-emerald-500/30 rounded-3xl space-y-4 shadow-md text-white">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    <h4 className="font-bold text-sm text-white flex items-center gap-2">
                      <Server className="w-4 h-4 text-emerald-400" />
                      <span>Neon Serverless PostgreSQL (Netlify Database)</span>
                    </h4>
                    <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 font-extrabold text-[10px] uppercase tracking-wider flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3 text-emerald-400" />
                      Netlify Partner
                    </span>
                  </div>
                  <p className="text-xs text-slate-300 mt-1 max-w-xl">
                    High-performance serverless PostgreSQL with scale-to-zero, instant database branching, and automatic <code className="text-emerald-300 font-mono text-[11px]">DATABASE_URL</code> injection on Netlify.
                  </p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className="px-3.5 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-xs transition cursor-pointer"
                  >
                    <Layers className="w-3.5 h-3.5" />
                    <span>Open Neon Hub</span>
                  </button>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1 text-slate-200">
                <div className="p-3 bg-white/5 backdrop-blur-xs rounded-2xl border border-white/10 space-y-1">
                ...
                <div className="p-3 bg-white/5 backdrop-blur-xs rounded-2xl border border-white/10 space-y-1">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Netlify Setup</span>
                  <div className="text-xs font-semibold text-emerald-300 truncate">
                    Integrations &gt; Neon (1-click)
                  </div>
                </div>
                <div className="p-3 bg-white/5 backdrop-blur-xs rounded-2xl border border-white/10 space-y-1">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Connection Variable</span>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    type="button"
                    onClick={() => setShowDbHubModal(true)}
                    className="px-3 py-1.5 bg-white/90 border border-emerald-200 hover:bg-emerald-50 rounded-xl text-xs font-bold text-emerald-900 flex items-center gap-1.5 shadow-2xs transition cursor-pointer"
                  >
